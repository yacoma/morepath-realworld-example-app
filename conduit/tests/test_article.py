...
def test_update_article():
    ...
    update_article_json = json.dumps({
        "article": {
            "title": "Another story"
        }
    })
    ...
    response = c.put(
        '/articles/test-text', update_article_json, headers=headers
    )
    assert article.items() <= response.json['article'].items()
    with db_session:
    ...
    update_article_json = json.dumps({
        "article": {
            "tagList": ["another", "story"]
        }
    })

    response = c.put(
        '/articles/another-story', update_article_json, headers=headers
    )
    ...
    response = c.put(
        '/articles/another-story', update_article_json,
        headers=headers, status=422
    )
    assert response.json == {
        'errors': {'body': ['must be of string type']}
    }
